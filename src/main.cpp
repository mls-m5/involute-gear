...
int main(int argc, char **argv) {
    ...
    auto lineOfContact = std::vector<glm::vec2>{};

    auto contactLength = 20.f;

    for (float i = -contactLength; i < contactLength; i += .7f) {
        lineOfContact.push_back(glm::vec2{i / 2.f, i});
    }
    ...
    for (auto event = std::optional<sdl::Event>{};;
        ...
        for (size_t i = 1; i < lineOfContact.size(); ++i) {
            auto p1 = lineOfContact.at(i - 1) + center;
            auto p2 = lineOfContact.at(i) + center;
        ...
        contactPointIndex = (contactPointIndex + 1) % lineOfContact.size();

        auto contactPoint = lineOfContact.at(contactPointIndex) + center;
